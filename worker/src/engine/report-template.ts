export const REPORT_HEAD = `<!DOCTYPE html>
...
export const HTML_SYSTEM_PROMPT = `You generate body content for a professional HTML report using the Adobe Spectrum design system.
...
export function buildReport(content: string, title: string): string {
  const head = REPORT_HEAD.replace("%%TITLE%%", escapeHtml(title));
...
export function cleanLlmContent(raw: string): string {
  let text = raw.trim();
